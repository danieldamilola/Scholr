  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (book?.storage_path) {
        await supabase.storage.from('library-books').remove([book.storage_path])
      }
  ...
  if (user?.profile?.role !== 'librarian') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        ...
        <p className="text-sm text-zinc-500">Only librarians can manage books.</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold text-zinc-900 mb-1">My Books</h1>
          <p className="text-sm text-zinc-500">Manage books you uploaded to the library.</p>
        </div>
        <Link
          href="/library/upload"
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm font-medium text-zinc-900 mb-1">No books uploaded yet</p>
          <p className="text-xs text-zinc-400 mb-4">Start by uploading a book to the library.</p>
          <Link
            href="/library/upload"
            className="inline-flex items-center gap-2 h-9 px-4 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-md transition-colors"
          >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-zinc-200 bg-zinc-50">
                <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-500 uppercase tracking-wide">Book</th>
                ...
                <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-500 uppercase tracking-wide hidden lg:table-cell">Downloads</th>
                <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-500 uppercase tracking-wide hidden lg:table-cell">Added</th>
                <th className="text-right py-3 px-4 text-xs font-semibold text-zinc-500 uppercase tracking-wide">Actions</th>
              </tr>
            </thead>
            <tbody>
              {books.map(book => (
                <tr key={book.id} className="border-b border-zinc-100 last:border-0 hover:bg-zinc-50 transition-colors">
                  <td className="py-3 px-4">
                    <p className="text-sm font-medium text-zinc-900 truncate max-w-[220px]">{book.title}</p>
                    {book.author && <p className="text-xs text-zinc-400">{book.author}</p>}
                  </td>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Are you sure you want to permanently delete <strong>{book.title}</strong>? This cannot be undone.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          ...
                          >
                            {isDeleting ? <Loader2 className="size-4 animate-spin" /> : 'Delete Book'}
                          </AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
