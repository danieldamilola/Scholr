...
export function BookUploadForm() {
  ...
  const validateCoverFile = (selectedFile: File): string | null => {
    if (!selectedFile.type.startsWith('image/')) return 'Cover must be an image (PNG, JPG)'
    if (selectedFile.size > 5 * 1024 * 1024) return 'Cover size must be less than 5MB'
    return null
  }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (err) { setErrors((prev) => ({ ...prev, file: err })); setFile(null); return }
  ...
  const handleCoverChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (err) { setErrors((prev) => ({ ...prev, cover: err })); setCover(null); return }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: urlData } = supabase.storage.from('library-books').getPublicUrl(filePath)
      ...
      if (cover) {
        ...
        const { data: coverUrlData } = supabase.storage.from('library-covers').getPublicUrl(coverPath)
      ...
      // Reset
      setTitle(''); setAuthor(''); setDescription(''); setCollege('')
      setDepartment(''); setSubject(''); setFile(null); setCover(null)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {/* Book Details */}
      <p className="text-zinc-400 text-xs font-medium uppercase tracking-wide">Book Details</p>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <Select value={department} onValueChange={setDepartment} disabled={!college || isLoading}>
            <SelectTrigger>
      ...
      {/* File Uploads */}
      <p className="text-zinc-400 text-xs font-medium uppercase tracking-wide pt-1">Files</p>
          ...
          {file ? (
            ...
            <>
              ...
              <p className="text-sm text-zinc-600 mb-1">
                ...
                <label htmlFor="bookFileInput" className="text-blue-600 hover:text-blue-700 cursor-pointer font-medium">
                  browse
                </label>
              </p>
      ...
      <div>
        <label className={labelCls}>Cover Image <span className="text-zinc-400 font-normal">(Optional)</span></label>
        <div
        ...
        >
          {cover ? (
            <div className="flex items-center justify-center gap-3">
              <ImageIcon className="size-5 text-zinc-500 shrink-0" />
              <span className="text-sm text-zinc-700 truncate max-w-xs">{cover.name}</span>
              <button
                type="button"
            ...
            <>
              <ImageIcon className="size-7 text-zinc-300 mx-auto mb-2" strokeWidth={1.5} />
              <p className="text-sm text-zinc-600 mb-1">
                Drag a cover image here, or{' '}
                <label htmlFor="coverInput" className="text-blue-600 hover:text-blue-700 cursor-pointer font-medium">
                  browse
                </label>
              </p>
              <p className="text-xs text-zinc-400">PNG, JPG, JPEG — max 5MB</p>
            </>
          )}
          <input
            id="coverInput"
